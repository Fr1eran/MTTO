...
def cal_regions(above_curves_list, below_curves_list):
    ...
    for i in range(len(above_curves_list)):
        ...
        x_cross, y_cross = find_2curves_crosspoint(
            above_curves_list[i], below_curves_list[i]
        )
        cross_points[0, i] = x_cross
        cross_points[1, i] = y_cross
        x_cur_a, y_cut_a = cut_curve_by_crosspoint(x_arr_a, y_arr_a, x_cross)
        x_cut_b, y_cut_b = cut_curve_by_crosspoint(x_arr_b, y_arr_b, x_cross)
...
def pad_2curve_lists(curves_list1, curves_list2):
    ...
    for i in range(len(curves_list1)):
        ...
        curve1_x, curve1_y, curve2_x, curve2_y = pad_2curves(
